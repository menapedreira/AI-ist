...
class finiteMDP:
    ...
    def traces2Q(self, trace):
        alpha = 0.2 # quanto mais pequeno este valor maior tera de ser o "n" no runPolicy
        nQ = np.zeros((self.nS,self.nA))

        while True:
            nQ = np.copy(self.Q)
            for i in trace:
                nQ[int(i[0]),int(i[1])] += alpha * (int(i[3]) + self.gamma*(max(nQ[int(i[2]),:]))-nQ[int(i[0]),int(i[1])])
            err = np.linalg.norm(self.Q-nQ)
            self.Q = np.copy(nQ)
            if err<1e-2:
                break
    ...
    def policy(self, x, poltype = 'exploration', par = []):
        ...
        if poltype == 'exploitation':
            #pass  escolho a acao que me da mais reward
            a = par[x].argmax()

            
        elif poltype == 'exploration':
            a = random.randint(0,self.nA-1) #qualquer acao disponivel
